        ...
        {CATEGORIES.map((c) => (
          <button
            ...
            className={`text-xs px-3 py-1.5 rounded-full border font-medium transition-all cursor-pointer ${
              c === active
                ? "gradient-bg text-white border-transparent shadow-lg shadow-brand/20"
        ...
        <div className="space-y-6">
          {filtered.map((post) => (
            <Link
              key={post.slug}
              href={`/blog/${post.slug}`}
              className="group block card-glass p-6 border border-brand/20 hover:border-brand/30 transition-all hover:-translate-y-0.5"
            >
              ...
              <h2 className="text-base font-bold text-white mb-2 group-hover:text-brand transition-colors leading-snug">
                {post.title}
              </h2>
              <p className="text-sm text-slate-400 leading-relaxed mb-4">{post.excerpt}</p>
              <div className="flex flex-wrap gap-2">
                {post.tags.slice(0, 4).map((t) => (
                  <span key={t} className="text-xs px-2 py-0.5 rounded-full bg-brand/20 text-brand border border-brand/30">
                    #{t}
                  </span>
                ))}
              </div>
